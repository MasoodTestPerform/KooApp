...
public class LoginKooTest extends BaseTest {
	@Test(description = "[TC_006] - Verify user is able to login with phone number", groups = { "regression",
	...
	public void TC_006_verifyUserLoginWithOTP() throws Exception {
		...
		try {
			BaseTest.LOGGER.get().logTestStep(extentTest.get(), "INFO",
					"<b><----Test case execution started for Test: TC_006 - Verify user is able to login with phone number ------><b>",
					false, BaseTest.mobileDriver.get());

			LoginKooPage loginKooPage = new LoginKooPage();
			loginKooPage.loginValidUser("English", TestConfig.getInstance().getUserName());
			BaseTest.LOGGER.get().logTestStep(extentTest.get(), "INFO",
					"<b><----Test case execution completed for Test:Successfully executed - TC_006 - Verify user is able to login with phone number ------><b>",
			...
			BaseTest.utilObj.get().getAssertManager().sAssertException(
					"Something went wrong in verifyUserLoginWithOTP. Exception:" + e.getMessage(), true,
