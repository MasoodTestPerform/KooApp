...
//	}
	...
	@Test(dataProvider = "Languages", dataProviderClass = LanguageDataProvider.class, description = "[TC_050, TC_051] Verify Topics tab should have Topics pills with + button, tapping on it or topic name, topic is followed>tapping on arrow button, user goes to Topics Details screen & related topics content should be shown", groups = {
			"regression", "Topics" })
	public void TC_050_051_verifyTopicTabPage(String language) {
		BaseTest.LOGGER.get().logTestStep(extentTest.get(), "INFO",
				"<b><----Test case execution started for Test: TC_050_051 - Verify Topics tab should have Topics pills with + button, tapping on it or topic name, topic is followed>tapping on arrow button, user goes to Topics Details screen & related topics content should be shown ------><b>",
		...
		try {
			...
			BaseTest.LOGGER.get().logTestStep(extentTest.get(), "INFO",
					"<b><----Test case execution completed for Test:Successfully executed - TC_050_051 - Verify Topics tab should have Topics pills with + button, tapping on it or topic name, topic is followed>tapping on arrow button, user goes to Topics Details screen & related topics content should be shown ------><b>",
			...
			BaseTest.utilObj.get().getAssertManager().sAssertException(
					"Something went wrong in TC_134 - Verify Topics Tab Position On Home Page. Exception:"
							+ e.getMessage(),
					true, BaseTest.mobileDriver.get());
			BaseTest.afterMethodDriver.set(BaseTest.mobileDriver.get());
		}
	...
	public void TC_053_verifyTopicsWidgetInFeedPage(String language) {
		BaseTest.LOGGER.get().logTestStep(extentTest.get(), "INFO",
				"<b><----Test case execution started for Test: TC_053 - Verify Topics widget in feed screen with same pills like in Topics tab with same behaviour. ------><b>",
				false, BaseTest.mobileDriver.get());
		try {
			LanguageDataProvider.setCurrentLanguage(language);
			LoginKooPage loginKooPage = new LoginKooPage();
			...
			BaseTest.LOGGER.get().logTestStep(extentTest.get(), "INFO",
					"<b><----Test case execution completed for Test:Successfully executed - TC_053 - Verify Topics widget in feed screen with same pills like in Topics tab with same behaviour. ------><b>",
					true, BaseTest.mobileDriver.get());
			BaseTest.afterMethodDriver.set(BaseTest.mobileDriver.get());
		} catch (Exception e) {
			BaseTest.utilObj.get().getAssertManager().sAssertException(
					"Something went wrong in TC_053 - Verify Topics widget in feed screen with same pills like in Topics tab with same behaviour. Exception:"
							+ e.getMessage(),
					true, BaseTest.mobileDriver.get());
			BaseTest.afterMethodDriver.set(BaseTest.mobileDriver.get());
		}
	...
	@Test(dataProvider = "Languages", dataProviderClass = LanguageDataProvider.class, description = "[TC_054, TC_055] Verify Manage Topics screen in own profile, 'All' & 'Following' tabs should be present in that screen._Verify all the topics in all tab & only followed topics in following tab.", groups = {
			"regression", "Topics" })
	public void TC_054_055_verifyManageTopicsPage(String language) {
		BaseTest.LOGGER.get().logTestStep(extentTest.get(), "INFO",
				"<b><----Test case execution started for Test: TC_054 - Verify Manage Topics screen in own profile, 'All' & 'Following' tabs should be present in that screen. ------><b>",
				false, BaseTest.mobileDriver.get());
		try {
			LanguageDataProvider.setCurrentLanguage(language);
			LoginKooPage loginKooPage = new LoginKooPage();
			...
			BaseTest.LOGGER.get().logTestStep(extentTest.get(), "INFO",
					"<b><----Test case execution completed for Test:Successfully executed - TC_054 - Verify Manage Topics screen in own profile, 'All' & 'Following' tabs should be present in that screen. ------><b>",
					true, BaseTest.mobileDriver.get());
			BaseTest.afterMethodDriver.set(BaseTest.mobileDriver.get());
		} catch (Exception e) {
			BaseTest.utilObj.get().getAssertManager().sAssertException(
					"Something went wrong in TC_054 - Verify Manage Topics screen in own profile, 'All' & 'Following' tabs should be present in that screen. Exception:"
							+ e.getMessage(),
					true, BaseTest.mobileDriver.get());
			BaseTest.afterMethodDriver.set(BaseTest.mobileDriver.get());
		}
...
//		loginKooPage.loginValidUser(language, TestConfig.getInstance().getUserName());
//		TopicsTab topics = new TopicsTab();
//		topics.allAndFollowTopicsInTopicsTab(LanguageDataProvider.getLanguageDataValue("TopicsTabText"),
//				"Topics You Follow");
//	}
